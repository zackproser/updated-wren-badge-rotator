...
// and copies this image to a local file first. Next, it uploads the local file to a special S3 prefix /extracted for
...
func copyExtractedBadgeImageToS3(badgeImageURL string) error {
	s, err := session.NewSession(&aws.Config{Region: aws.String(S3_REGION)})
	if err != nil {
		fmt.Printf("Error creating S3 session: %+v\n", err)
		return err
	...
	response, err := http.Get(badgeImageURL)
	if err != nil {
		return err
	}
