...
var (
	...
	WrenBadgeURL = fmt.Sprintf("https://www.wren.co/profile/%s", os.Getenv("WREN_USERNAME"))
...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	fmt.Printf("Attempting to fetch wren URL: %s\n", WrenBadgeURL)
	...
	badgeImageURL, resizeErr := extractBadgeFromWrenProfile()
	if resizeErr != nil {
	...
	copyErr := copyExtractedBadgeImageToS3(badgeImageURL)
	if copyErr != nil {
